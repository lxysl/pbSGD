...
def main():
    ...
    if args.multiprocessing_distributed:
        if args.wandb:
            wandb.init(project="pbAdam", name=cur_time, config=vars(args), group="DDP")
            print(vars(args))
...
def get_optimizer(args, net):
    ...
    if args.optim == 'SGD':
        return torch.optim.SGD(net.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    elif args.optim == 'Adam':
        return torch.optim.Adam(net.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    elif args.optim == 'RMSprop':
        return torch.optim.RMSprop(net.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    elif args.optim == 'Adagrad':
        return torch.optim.Adagrad(net.parameters(), lr=args.lr, weight_decay=args.weight_decay)
        ...
        return torch.optim.Adamax(net.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    elif args.optim == 'AMSGrad':
        return torch.optim.Adam(net.parameters(), lr=args.lr, weight_decay=args.weight_decay, amsgrad=True)
...
def validate(val_loader, model, criterion, epoch, writer, args):
    ...
    if args.wandb:
        wandb.log({'val loss': losses.avg, 'val acc': top1.avg}, step=epoch)
